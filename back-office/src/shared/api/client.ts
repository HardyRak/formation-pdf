 ...
 */
import type { ApiError, UserDto } from '@/shared/types/api';
...
export const api = {
  ...
  postForm: <T>(path: string, form: FormData) => request<T>('POST', path, { body: form, isForm: true }),
  putForm: <T>(path: string, form: FormData) => request<T>('PUT', path, { body: form, isForm: true }),
