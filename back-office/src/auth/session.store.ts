...
import { api, configureHttpClient, isApiError, SESSION_KEY, toApiError, type StoredSession } from '../api/client';
import type { ApiError, AuthSessionDto, UserDto } from '../api/types';
...
export const useSessionStore = create<SessionState & {
  ...
  async function refreshSession(): Promise<string | null> {
    ...
    try {
      const session = await api.post<AuthSessionDto>('/auth/refresh', { refreshToken: current.refreshToken }, { anonymous: true });
      await persistSession(session);
      set({ session: { accessToken: session.accessToken, refreshToken: session.refreshToken, expiresAt: session.expiresAt }, user: session.user });
  ...
  return {
    ...
    async login(email: string, password: string): Promise<boolean> {
      ...
      try {
        const session = await api.post<AuthSessionDto>('/auth/login', { email: email.trim(), password }, { anonymous: true });
        // Le back-office est réservé aux responsables de formation.
        if (!isManager(session.user)) {
          set({ status: 'error', error: { status: 403, code: 'FORBIDDEN', message: "Ce compte n'est pas autorisé sur le back-office." }, session: null, user: null });
          await persistSession(null);
          return false;
        }
        await persistSession(session);
        set({
          ...
          session: { accessToken: session.accessToken, refreshToken: session.refreshToken, expiresAt: session.expiresAt },
          user: session.user,
          error: null,
          notice: null,
          bootstrapping: false,
