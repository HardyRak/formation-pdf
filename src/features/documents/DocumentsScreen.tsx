  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.bg }]} edges={['top']}>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          level ? (
            <View style={[styles.summary, { backgroundColor: theme.surface, borderColor: theme.border }, shadow(3)]}>
              <Text style={[styles.summaryText, { color: theme.textMuted }]}>{level.description}</Text>
              <ProgressBar percent={levelPercent} color={accent} showLabel label={'Progression du niveau'} />
            </View>
          ) : null
        }
        ListEmptyComponent={
        ...
        renderItem={({ item, index }) => {
          ...
          const status = !progress ? 'new' : progress.completed ? 'done' : 'reading';
          return (
            <Animated.View entering={FadeInDown.delay(Math.min(index, 6) * 60).duration(340)}>
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.card,
                  { backgroundColor: theme.surface, borderColor: theme.border, opacity: pressed ? 0.93 : 1 },
                  shadow(3),
                ]}
              >
